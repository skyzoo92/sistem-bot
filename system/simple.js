...
Silahkan Reply/Balas pesan ini dengan Angka ${list[0]?.[1] || 1} - ${list.length}
${options.noList || list.length === 0 ? "" : list.map((item, index) => {
  ...
  if (options?.contextInfo?.externalAdReply?.thumbnailUrl) {
    await conn.sendMessage(jid, {
      text: caption,
      contextInfo: options.contextInfo
    }, { quoted: m });
  } else if (image && Buffer.isBuffer(image)) {
    await conn.sendFile(jid, image, null, caption, m, false, false, smlcap);
  } else {
  ...
  conn.textOptions = async function (jid, text, image, list = [], m, options = {}, smlcap = { smlcap: true }) {
  if (!Array.isArray(list)) list = [];
...
Silahkan Reply/Balas pesan ini dengan *${list.length > 0 ? list[0][1] : ""}*${list.length > 1 ? ` atau *${list[1][1]}*` : ""}
 ...
 conn.textInput = async function (jid, text, image, input, m, options = {}, smlcap = { smlcap: true }) {
  let caption = `
${text}
  `.trim();
...
`.trim()
      ...
      conn.sendList = async (jid, name, sections, quoted, json) => {
  if (!Array.isArray(sections)) throw new Error("Sections harus berupa array");

  const listMessage = {
    title: name,
    sections: sections // langsung pakai struktur dari parameter
  };
          ...
          viewOnceMessage: {
            message: {
              ...
              interactiveMessage: proto.Message.InteractiveMessage.create({
                ...
                contextInfo: {
                  mentionedJid: [...conn.parseMention(json.body || "")],
                  ...(quoted ? {
...
/*exports.smsg = (conn, m, hasParent) => {
    ...
    const ppUrl = await conn
      ...
      .catch((_) => "https://files.catbox.moe/a1wkxg.jpg");
    let bio;
    try {
    ...
    global.fkontak = {
      ...
      message: {
        conversation: "© *Rapthalia Multi Device*",
      },
    };
    const a = {
      contextInfo: {
